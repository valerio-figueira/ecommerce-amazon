...
type CollectionListViewProps = {
  ...
  onEdit: (item: AdminCollectionSummary) => void;
  onDelete: (item: AdminCollectionSummary) => void;
};
...
export function CollectionListView({
  ...
  onEdit,
  onDelete,
}: CollectionListViewProps): React.JSX.Element {
  if (items.length === 0) {
    return (
      <p className="py-8 text-center text-sm text-[var(--admin-text-muted)]">
        Nenhuma coleção cadastrada. Crie a primeira para montar guias temáticos.
      </p>
    );
  }
        ...
        <li key={item.id} className="cms-block-card cms-block-card--plain">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0 flex-1">
              <p className="font-semibold text-[var(--admin-navy)]">{item.title}</p>
              <p className="mt-0.5 text-xs text-[var(--admin-text-muted)]">
                /colecoes/{item.slug} · {item.productCount} produto
                {item.productCount === 1 ? '' : 's'} · Atualizado em {formatDate(item.updatedAt)}
              </p>
            </div>
