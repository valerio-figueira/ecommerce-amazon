...
export function validateAvatarImage(buffer: Buffer, mime: string): void {
  const normalizedMime = mime.toLowerCase().trim();

  if (buffer.length < 1 || buffer.length > AVATAR_MAX_BYTES) {
...
export function mimeToAvatarExtension(mime: string): string {
  const normalizedMime = mime.toLowerCase().trim();
