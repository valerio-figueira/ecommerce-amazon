...
| Toolbar      | H3, negrito/itálico/riscado, listas, tabela, link, **Produto** (`/produto` ou botão na toolbar)                        |
| Embeds       | Shortcode `[[product:slug]]` no HTML; chips no modo Visual; modal de busca no catálogo local (mesmo fluxo dos artigos) |
...
| Vitrine      | `ProductLongDescription` em `/produtos/[slug]` resolve shortcodes e aplica auto-links em `longDescriptionHtml`         |
