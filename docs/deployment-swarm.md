...
| `docker.service` failed / `Job for docker.service failed`        | `live-restore: true` em `/etc/docker/daemon.json` — incompatível com Swarm                                                                         | Remover `live-restore`; `systemctl restart docker`; redeploy stack                                                                                                   |
...
| `EHOSTUNREACH 10.0.x.x` api/worker → postgres/redis              | UFW `DOCKER-USER` bloqueava east-west overlay (só 80/443 liberados)                                                                                | Como root: `bash /opt/vitrine/deploy/scripts/patch-ufw-docker-overlay.sh`; redeploy                                                                                  |
