...
| Migrate `EHOSTUNREACH 10.0.x.x:5432`                             | `DATABASE_URL` no secret ou `.env` com IP overlay stale                                                                                            | `render-env.sh` + `migrate.sh` usam `apply_swarm_overlay_urls` (`postgres:5432`); remover secret `DATABASE_URL` do GitHub se existir                                 |
