...
**Excluído do sitemap:** `/cupons` (sem página web), `/go/*`, URLs com query params, comparações UGC (`draft` ou sem slug). **Incluído:** `/comparar/{slug}` com `status=published` — ver [admin-comparisons-phase1.md](./admin-comparisons-phase1.md).
